...
async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    if count == 0 {
        ...
        return Ok(Response::builder()
            ...
            .body(json!({
                "statusAPI": "ERROR", 
                "message": format!("Synonym with UUID {} not found", synonym_input.synonym_uuid)
            }).to_string().into())
            .map_err(Box::new)?);
    }
    ...
    match client.execute(delete_query, &[&synonym_input.synonym_uuid]).await {
        Ok(rows_affected) => {
            ...
            if rows_affected == 0 {
                ...
                return Ok(Response::builder()
                    ...
                    .body(json!({
                        "statusAPI": "ERROR",
                        "message": format!("No synonym with UUID {} found", synonym_input.synonym_uuid)
                    }).to_string().into())
                    .map_err(Box::new)?);
            }
        ...
        Err(e) => {
            ...
            Ok(Response::builder()
                ...
                .body(json!({
                    "statusAPI": "ERROR", 
                    "message": format!("Failed to delete synonym: {}", e)
                }).to_string().into())
                .map_err(Box::new)?)
        }
    }
