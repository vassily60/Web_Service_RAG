...
struct AddRecurrentQueryRequest {
    recurrent_query_name: String,
    query_type: String,
    query_content: String,
    query_tags: Option<String>,
    query_start_document_date: Option<String>, // Format: YYYY-MM-DD
    query_end_document_date: Option<String>,   // Format: YYYY-MM-DD
...
async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    // Parse request body
    let body = event.body();
    let request_data: AddRecurrentQueryRequest = match serde_json::from_slice(body.as_ref()) {
        Ok(data) => data,
        Err(e) => {
    ...
    if request_data.recurrent_query_name.is_empty() || request_data.query_type.is_empty() || 
       request_data.query_content.is_empty() {
        return Ok(Response::builder()
            .status(400)
            .header("content-type", "application/json")
    ...
    // Generate UUID for the new recurrent query
    let recurrent_query_uuid = Uuid::new_v4().to_string();
    
    // Get the current time for creation timestamp
    let current_time = Utc::now();
    
    // Parse date strings if provided
    let start_date = if let Some(date_str) = &request_data.query_start_document_date {
            ...
            match chrono::NaiveDate::parse_from_str(date_str, "%Y-%m-%d") {
                ...
                Err(_) => {
                    return Ok(Response::builder()
                        .status(400)
                        .header("content-type", "application/json")
                        .body(json!({"statusAPI": "ERROR", "message": "Invalid start date format. Use YYYY-MM-DD"}).to_string().into())
                        .map_err(Box::new)?);
                }
            }
        }
    ...
    let end_date = if let Some(date_str) = &request_data.query_end_document_date {
        if date_str.is_empty() {
            None
        } else {
            match chrono::NaiveDate::parse_from_str(date_str, "%Y-%m-%d") {
                ...
                Err(_) => {
                    return Ok(Response::builder()
                        .status(400)
                        .header("content-type", "application/json")
                        .body(json!({"statusAPI": "ERROR", "message": "Invalid end date format. Use YYYY-MM-DD"}).to_string().into())
                        .map_err(Box::new)?);
                }
            }
        }
    ...
    // Create insert query
    let insert_query = "
        INSERT INTO document_library.recurrent_queries_extended (
    ...
    let insert_result = client.execute(
        ...
        &[
            ...
            &user_email, // Using email extracted from token instead of user_uuid
    ...
    match insert_result {
        Ok(_) => {
            ...
            let response_body = RecurrentQueryResponse {
                ...
                message: "Recurrent query added successfully".to_string(),
                recurrent_query_uuid,
            };
        ...
        Err(e) => {
            eprintln!("Failed to add recurrent query: {}", e);
            Ok(Response::builder()
                .status(500)
                .header("content-type", "application/json")
                .body(json!({"statusAPI": "ERROR", "message": format!("Failed to add recurrent query: {}", e)}).to_string().into())
                .map_err(Box::new)?)
        }
    }
}
