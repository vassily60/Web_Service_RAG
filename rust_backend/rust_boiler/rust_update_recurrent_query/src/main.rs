...
struct RecurrentQueryRequest {
    ...
    query_start_document_date: Option<String>,
    query_end_document_date: Option<String>,
...
async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    if request_data.recurrent_query_name.is_empty() {
        return Ok(Response::builder()
            .status(400)
            .header("content-type", "application/json")
            .body(json!({"statusAPI": "ERROR", "message": "Missing required recurrent_query_name"}).to_string().into())
            .map_err(Box::new)?);
    }

    if request_data.query_type.is_empty() {
        return Ok(Response::builder()
            .status(400)
            .header("content-type", "application/json")
            .body(json!({"statusAPI": "ERROR", "message": "Missing required query_type"}).to_string().into())
            .map_err(Box::new)?);
    }
    
    if request_data.query_content.is_empty() {
        return Ok(Response::builder()
            .status(400)
            .header("content-type", "application/json")
            .body(json!({"statusAPI": "ERROR", "message": "Missing required query_content"}).to_string().into())
            .map_err(Box::new)?);
    }
    ...
    // Check if recurrent query exists before trying to update it
    ...
    let now = Utc::now();
    // Convert the timestamp to f64 instead of i64 to match PostgreSQL's float8 type
    let updated_timestamp = now.timestamp() as f64;
    
    // Process date strings to handle PostgreSQL date format
    // We need to let PostgreSQL handle the date conversion explicitly in the SQL query
    let start_date_sql = request_data.query_start_document_date.clone();
    let end_date_sql = request_data.query_end_document_date.clone();
    
    // Update the recurrent query with explicit type casting in SQL
    let update_query = "
        UPDATE document_library.recurrent_queries_extended
        SET 
            recurrent_query_name = $1,
            query_type = $2,
            query_content = $3,
            query_tags = $4,
            query_start_document_date = CASE WHEN $5::text IS NULL THEN NULL ELSE $5::text::date END,
            query_end_document_date = CASE WHEN $6::text IS NULL THEN NULL ELSE $6::text::date END,
            comments = $7,
            updated_date = TO_TIMESTAMP($8::float8),
            updated_by = $9
        WHERE recurrent_query_uuid = $10
    ";
    
    match client.execute(update_query, &[
        &request_data.recurrent_query_name,
        &request_data.query_type,
        &request_data.query_content,
        &request_data.query_tags,
        &start_date_sql,
        &end_date_sql,
        &request_data.comments,
        &updated_timestamp,
        &user_email,
        &request_data.recurrent_query_uuid
    ]).await {
        Ok(count) => {
            if count > 0 {
                // Success response
                let response_body = RecurrentQueryResponse {
                    ...
                    message: "Recurrent query updated successfully".to_string(),
                    recurrent_query_uuid: request_data.recurrent_query_uuid,
                };
        ...
        Err(e) => {
            eprintln!("Failed to update recurrent query: {}", e);
            Ok(Response::builder()
                .status(500)
                .header("content-type", "application/json")
                .body(json!({"statusAPI": "ERROR", "message": format!("Failed to update recurrent query: {}", e)}).to_string().into())
                .map_err(Box::new)?)
        }
    }
}
